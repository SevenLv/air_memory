...
v1.2.0 修复了五个影响用户体验的问题: 启动脚本显示的 UI 地址在浏览器无法直接访问、系统各处均未显示版本号、.gitignore 缺失运行时目录配置、Windows 上随 CMD 控制台输出浏览器任务栏不断闪烁、以及中文内容在 Windows 上被损坏为问号. 本次发布不包含任何 Breaking Change, 直接升级即可.
...
### 5. 记忆内容中文显示为问号 (Issue #32)

**问题描述**

通过 MCP 工具存储包含中文的记忆后, 在 Web 管理界面查看时中文内容显示为问号 (`?`). AI 通过 MCP 工具查询时, 返回的内容中同样存在问号.

**根因分析**

两个根因:

1. `start.bat` 缺失 `PYTHONUTF8=1` 环境变量. `chcp 65001` 仅修改 CMD 控制台的 OEM 代码页, 不影响 Python 的 `locale.getpreferredencoding()`. 在西欧语言版 Windows（如 CP1252 代码页）上, Python 会将中文字符用 ANSI 代码页编码, 无法映射的字符被替换为 `?`.

2. MCP `query_memory` 工具返回 `list[dict]` 时, MCP SDK 会将每条记忆拆分为独立的 `TextContent` 块, 部分 AI 客户端在处理多块 `TextContent` 时兼容性较差, 可能丢失数据或产生乱码.

**修复方案**

- `start.bat` 和 `start.sh`: 追加 `PYTHONUTF8=1` 和 `PYTHONIOENCODING=utf-8` 环境变量设置.
- `backend/src/air_memory/mcp/server.py`: `query_memory` 工具返回类型由 `list[dict]` 改为 `str`（`json.dumps(results, ensure_ascii=False)`）, 合并为单个 `TextContent`, 确保中文字符直接输出而非 `\uXXXX` 转义.
- `backend/src/air_memory/main.py`: 服务启动时检测 Windows 上 UTF-8 模式是否开启, 若未开启则输出 WARNING 便于排查.
...
| 文件 | 变更类型 | 说明 |
| --- | --- | --- |
| `start.bat` | 修复 | uvicorn host 改为 `127.0.0.1`; Banner 新增版本号 v1.2.0; 新增 `--no-access-log`; 新增 `PYTHONUTF8=1` 和 `PYTHONIOENCODING=utf-8` |
| `start.sh` | 修复 | uvicorn host 改为 `127.0.0.1`; Banner 新增版本号 v1.2.0; 新增 `--no-access-log`; 新增 `PYTHONUTF8=1` 和 `PYTHONIOENCODING=utf-8` |
| `backend/src/air_memory/main.py` | 修复 | 新增 `APP_VERSION = "1.2.0"` 常量; FastAPI 元数据引用常量; 新增 `GET /api/v1/version` 端点; Windows UTF-8 模式启动检测 |
| `backend/src/air_memory/mcp/server.py` | 修复 | `query_memory` 返回 JSON 字符串 (`ensure_ascii=False`)，替代 `list[dict]` |
