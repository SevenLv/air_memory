...
| 1.2 | 2026-4-9 | 将各方案架构图由 ASCII 替换为 Mermaid 图；更新技术需求分析以反映分级存储、磁盘上限等新性能需求 |
| 1.3 | 2026-4-9 | 新增参考文档表；重写技术需求分析，直接引用 SRD v1.0 各条需求编号；结论章节更新对 SRD 的说明 |
...
本章根据系统需求文档（`/doc/srd_v1.0.md`）提炼关键技术驱动需求，并标注对应的需求编号，以便后续追踪技术决策与系统需求之间的映射关系。

| 需求类别 | 具体要求 | 来源需求编号 |
| --- | --- | --- |
| 部署平台 | 支持 macOS 和 Windows 本地一键部署，系统在操作系统启动后自动运行 | FR-DEP-001, FR-DEP-002, FR-DEP-003, FR-DEP-004 |
| AI Agent 接口协议 | 向 AI Agent 提供记忆存储接口、记忆查询接口和记忆价值反馈接口 | FR-API-001, FR-API-002, FR-API-003, FR-API-006 |
| 查询模式 | 支持快速查询（仅高速存储层，≤ 100ms）和深度查询（高速层 + 普通层，无时间限制） | FR-API-004, FR-API-005 |
| 分级存储与迁移 | 根据记忆价值评分实现分级存储，驱动高速层与普通层之间的自动迁移 | FR-API-007 |
| 性能指标（响应时间） | 记忆存储端到端响应 ≤ 100ms；快速查询端到端响应 ≤ 100ms；深度查询无硬性时间限制 | PR-001, PR-002, PR-003 |
| 性能指标（内存上限） | 系统整体内存占用 ≤ 8GB；分级存储在上限内最大化高速存储层覆盖容量 | PR-004, PR-006, PR-007, PR-008 |
| 性能指标（磁盘上限） | 系统磁盘占用 ≤ 40GB；在触及上限前自动删除低价值记忆中最旧的数据（168 小时内记忆受保护） | PR-005, PR-009, PR-010, PR-011 |
| 管理 UI | 供人类使用的 Web UI，支持查询/删除记忆、查看存储日志、查询日志、价值反馈记录与综合价值评分 | FR-UI-001, FR-UI-002, FR-UI-003, FR-UI-004, FR-UI-005, FR-UI-006 |
| 文档 | 提供完整的部署手册和用户手册 | FR-DOC-001, FR-DOC-002 |
...
graph TB
    ...
    Agent -->|MCP Protocol| Backend
    Agent -->|REST API HTTP/JSON| Backend

    subgraph Backend["FastAPI Backend (Python 3.11+)"]
        direction TB
        API["REST API / MCP Server"]
        MemSvc["MemoryService + FeedbackService"]
    end

    API --> MemSvc
    MemSvc --> ChromaDB[("ChromaDB\n向量存储")]
    MemSvc --> SQLite[("SQLite\n操作日志 + 价值评分")]
...
graph TB
    ...
    subgraph Backend["Go + Gin Backend（单一可执行文件）"]
        direction TB
        API["REST API / MCP Server"]
        StaticRes["内嵌 Vue.js 静态资源"]
    end

    API --> SQLiteVec[("SQLite + sqlite-vec\n向量存储 + 日志")]
    WebUI["浏览器\n管理界面"] -->|HTTP/JSON| StaticRes
    StaticRes -->|API 调用| API
```
...
在阅读系统需求文档（`/doc/srd_v1.0.md`）后，对本技术选型进行了重新评估，结论如下：

- 方案一所选技术栈可完整覆盖 SRD 中的全部功能需求（FR-DEP、FR-API、FR-UI、FR-DOC）和性能需求（PR-001 至 PR-011）；
- 技术选型无需调整，原方案一结论继续有效；
- 后续架构设计文档（SAD）将依据 SRD 需求编号进行需求分配。
