...
根据 `/doc/pdd_v1.2.md` 中的产品定义，提炼出以下关键技术需求：

| 需求类别 | 具体要求 |
| --- | --- |
| 部署平台 | 支持 macOS 和 Windows 本地一键部署，默认自启动 |
| 接口协议 | 向 AI Agent 提供记忆存储接口、记忆查询接口和记忆价值反馈接口 |
| 性能指标 | 记忆存储操作 ≤ 100ms；快速查询 ≤ 100ms；深度查询无时间限制 |
| 内存上限 | 系统整体内存占用 ≤ 8GB；分级存储（热层内存 + 冷层磁盘）在上限内最大化高速查询覆盖 |
| 磁盘上限 | 系统磁盘占用 ≤ 40GB；在触及上限前自动删除低价值记忆中最旧的数据 |
| 管理 UI | 供人类使用的 Web UI，支持查询/删除记忆、查看存取日志、查看 Feedback 日志与价值评分 |
| 文档 | 提供完整的部署手册和用户手册 |
