...
- **`service.py`**（待实现）：MemoryService 类
  ...
  - `save(content: str) -> str`：生成 Embedding，初始同时存入热层和冷层（value_score = INITIAL_VALUE_SCORE，默认 0.5），返回 memory_id
...
sequenceDiagram
    ...
    MemSvc->>Cold: (4) 存入冷层 PersistentClient<br/>初始 value_score=0.5（持久化）
    MemSvc->>Hot: (5) 存入热层 EphemeralClient<br/>初始 value_score=0.5（快速访问）
    MemSvc->>DB: (6) 创建 memory_values 记录<br/>(memory_id, value_score=0.5, tier=hot)
