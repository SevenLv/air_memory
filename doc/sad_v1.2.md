...
| ChromaDB 进程 + HNSW 索引 | ~200MB（基础）+ ~2MB / 千条记忆 | 随数据量线性增长（见下方说明） |
| SQLite + aiosqlite | < 10MB | 极低开销 |
| Docker 容器运行时开销 | ~100MB | Docker engine 本身 |
| **合计（最小工作集）** | **~1,000MB** | 仅含基础组件，不含数据 |

**ChromaDB HNSW 索引内存增长机制**：

ChromaDB 在服务启动时将 HNSW 向量索引**完整加载到内存**，索引大小随记忆条目数量线性增长，**不存在硬性上限**。每条记忆的内存消耗由以下部分组成：

| 组成部分 | 大小 | 说明 |
| --- | --- | --- |
| 向量数据 | ~1.5KB | 384 维 × 4 bytes（float32） |
| HNSW 图节点连接（M=16） | ~0.4KB | 每节点约 M×2 个 int32 邻居指针 |
| 文本内容 + 元数据 | ~0.1KB | 取决于原文长度 |
| **合计（每条记忆）** | **~2KB** | 即 ~2MB / 千条记忆 |

**不同数据量下的内存估算**：

| 记忆条目数 | HNSW 索引内存增量 | 总内存需求（含基础 1GB） |
| --- | --- | --- |
| 1 万条 | ~20MB | ~1.1GB |
| 10 万条 | ~200MB | ~1.2GB |
| 50 万条 | ~1GB | ~2GB |
...
| 生产环境（记忆条目 10 万 ~ 50 万条） | 4GB | 8GB |
| 生产环境（记忆条目 50 万 ~ 100 万条） | 8GB | 16GB |

> **数据规模管理建议**：对于 AI Agent 个人记忆系统，典型使用场景下记忆条目数量通常在数千至数万条（约 10～200MB 内存增量），处于合理可控范围。当记忆条目超过 **50 万条**时，建议：
> 1. **记忆老化策略**：为记忆条目设置 TTL（如 180 天），定期清理过期记忆；
> 2. **相似记忆合并**：对相似度超过阈值的记忆进行合并，减少冗余条目；
> 3. **分集合管理**：按 Agent 或时间段将记忆分散到不同 ChromaDB Collection，避免单一索引过大；
> 4. **升级到专用向量数据库**：如数据量持续增长至百万级，可考虑替换为支持分片的向量数据库（如 Qdrant、Milvus）。
