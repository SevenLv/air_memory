...
> **乱码徽章**：如果某条存储记录的原始内容疑似因历史编码问题损坏（内容中问号比例过高），该记录的"原始内容"列将显示橙色"乱码"徽章，鼠标悬停可查看说明。此为历史遗留数据问题，v1.2.0 起修复了根因（Windows 上 `PYTHONUTF8=1` 已正确配置），正常环境下新增的记忆不受影响。如历史数据显示大量问号但无乱码徽章，请升级至 v1.2.5。
