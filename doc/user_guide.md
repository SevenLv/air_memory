...
| 1.6 | 2026-4-15 | 2.3 节改为默认最近记忆列表+分页+筛选+详情；3.2.2 节新增记忆管理列表和记忆详情接口 |
...
1. 可按 `记忆 ID` 精确查询；
2. 可按 `时间范围` 查询；
3. 支持分页切换页码和每页条数（20 / 50 / 100）。

**进入详情**：

1. 在列表中点击任意一行，或点击"查看详情"；
2. 系统跳转到 `/memories/{memory_id}` 详情页；
3. 详情页展示完整字段，至少包括：
   - 记忆 ID；
   - 原始数据；
   - 提交时间；
...
```
GET /api/v1/memories/manage/list
```
...
```
GET /api/v1/memories/{memory_id}/detail
```
...
```json
{
  ...
  "tier": "hot",
  "feedback_count": 3,
  "value_updated_at": "2026-04-10T09:00:00Z",
  "message": "ok"
}
```
