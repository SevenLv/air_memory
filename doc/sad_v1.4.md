...
AIR_Memory 是一个为 AI Agent 设计的本地部署记忆系统。AI Agent 可通过 AIR_Memory 高效地存储记忆、查询相关记忆，并能对查询结果的价值进行反馈评价。系统通过分级存储架构（热层/冷层），在 8GB 内存上限和 40GB 磁盘上限约束下最大化高价值记忆的快速查询性能；磁盘空间触及上限时自动淘汰低价值最旧数据；同时向人类提供 Web 管理界面进行记忆查询、删除、日志查看和价值评分查询。
...
| DiskManager | 监控冷层磁盘占用，接近 40GB 上限时自动淘汰低价值记忆中 created_at 最早的数据 |
...
```
DiskManager（每小时定期执行，或在存储操作后触发）
  |
  +-- (1) 统计冷层 ChromaDB 数据目录 + SQLite 文件的总磁盘占用（GB）
  |
  +-- (2) 若当前占用 > DISK_BUDGET_GB（38GB）：
  |       |
  |       +-- (3) 从 SQLite `memory_values` 中，按
  |       |       `value_score ASC, created_at ASC` 排序，
  |       |       取出若干低价值最旧的 memory_id 列表
  |       |
  |       +-- (4) 对每个 memory_id，从冷层 ChromaDB 和
  |       |       SQLite 所有相关表中删除全部数据
  |       |
  |       +-- (5) 重新统计磁盘占用，重复直至占用 < DISK_SAFE_GB（35GB）
  |
  +-- 若当前占用 <= DISK_BUDGET_GB：不执行淘汰，直接返回
```
...
GET /api/v1/memories/{id}/feedback/logs 查看指定记忆的反馈历史
GET /api/v1/memories/{id}/value-score   查看指定记忆的当前价值评分
GET /api/v1/admin/tier-stats             查看热/冷层统计（数量 + 内存占用）
GET /api/v1/admin/disk-stats             查看当前磁盘占用
  |
  v
FastAPI REST API -> MemoryService / LogService / FeedbackService / DiskManager
```
