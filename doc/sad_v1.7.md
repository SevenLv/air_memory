...
graph LR
    ...
    Doc --> SAD["sad_v1.7.md<br/>系统架构设计说明书"]
```
...
| M1-AC-02 | `POST /api/v1/memories` 接口能正确接收记忆内容并返回 `memory_id`；端到端响应时间在预热后不超过 100ms（满足 PR-001） |
| M1-AC-03 | `GET /api/v1/memories?fast_only=true` 仅查询热层，端到端响应时间不超过 100ms（满足 PR-002） |
...
| M3-AC-05 | `MemoryService` 的存储和快速查询路径有明确的响应时间断言，验证性能不超过 100ms |
...
| M6-06 | 执行性能需求确认：使用工具测量记忆存储和快速查询的端到端响应时间，验证不超过 100ms | Wii |
| M6-07 | 执行资源占用确认：在容器运行时验证内存占用不超过 8GB（PR-004）、磁盘占用不超过 40GB（PR-005） | Wii |
| M6-08 | 汇总所有验证结果，输出系统确认报告，记录每条需求的验证状态（通过/不通过/阻塞） | Wii |
...
| M6-AC-04 | 记忆存储端到端响应时间实测不超过 100ms（PR-001） |
| M6-AC-05 | 快速查询端到端响应时间实测不超过 100ms（PR-002） |
| M6-AC-06 | 系统运行时内存占用实测不超过 8GB（PR-004） |
| M6-AC-07 | 系统磁盘占用实测不超过 40GB，DiskManager 在超过触发水位时能正确触发淘汰（PR-005, PR-009） |
| M6-AC-08 | Web 管理界面所有功能经验证均与用户手册描述一致（FR-UI-001~006） |
| M6-AC-09 | 系统确认报告已输出，无严重（Critical）级别的未修复缺陷 |
| M6-AC-10 | 若有非严重缺陷，需记录在报告中并由项目经理决策是否放行 |
