...
系统通过分级存储架构（热层/冷层），在 8GB 内存上限和 40GB 磁盘上限约束下最大化高价值记忆的快速查询性能。查询接口会为每次输入生成 `input_id` 并按"关联记忆优先 + 热层匹配 + 冷层匹配"策略返回最多 10 条结果，反馈接口基于 `input_id` 维护输入信息与记忆关联评分。磁盘空间触及上限时系统会自动淘汰低价值最旧数据（创建时间在 168 小时内的记忆受保护不得淘汰）。系统同时向人类提供 Web 管理界面进行记忆查询、删除、日志查看、价值评分查询和输入信息管理。
...
- **`server.py`**（待实现）：实现 MCP Server
  ...
  - Tool: `query_memory(query: str, top_k: int, fast_only: bool)` - 查询记忆，返回 `input_id` 及配额结果
...
- **`service.py`**（待实现）：MemoryService 类
  ...
  - `query(query: str, top_k: int, fast_only: bool) -> list[Memory]`：
    ...
    - `fast_only=False`：优先返回输入信息关联记忆（最多 5 条），再补充热层匹配（最多 3 条）和冷层匹配（最多 2 条），合并去重后总数最多 10 条
    - 每次查询生成并返回 `input_id`
...
- **`memory.py`**（待实现）：记忆相关 Pydantic 模型
  ...
  - `MemoryQueryRequest`（含 `top_k: int = 10`、`fast_only: bool = False`）、`MemoryQueryResponse`（含 `input_id`）、`Memory`（含 `source`、`association_score`）
...
sequenceDiagram
    ...
    API-->>Agent: {"input_id":"...","memories":[...], "count":N, "query_mode":"deep"}
...
| GET | `/memories` | 查询记忆 | Query: `query`, `top_k=10`, `fast_only=false` | `{"input_id":"string","memories":[...], "count": N, "query_mode": "fast"\|"deep"}` |
...
| `query_memory` | `query: str`, `top_k: int = 10`, `fast_only: bool = False` | 查询相关记忆并返回 `input_id`；`fast_only=False` 时采用关联5+热3+冷2配额策略 |
...
| M1-07 | 实现 MCP Server（`save_memory`、`query_memory`、`feedback_memory` 三个工具） | Neo | FR-API-001~008 |
| M1-08 | 将所有性能阈值设计为可配置项，通过环境变量或配置文件指定，默认值与 SRD v1.2 要求一致；可配置项包括：存储/查询响应时间上限（默认 100ms）、热层内存预算（默认 6GB）、磁盘触发水位（默认 38GB）、磁盘安全水位（默认 35GB）、磁盘上限（默认 40GB）、新记忆保护时长（默认 168 小时） | Neo | PR-001, PR-002, PR-004~011 |
...
| M4-06 | 更新环境变量配置说明文档（`doc/env_config.md`），新增 `STATIC_DIR`、`PORT`、`HF_HOME` 等新配置项 | Neo | PR-001, PR-002, PR-004~011 |
...
| M6-03 | 执行 AI Agent 接口需求确认（FR-API-001~008）：通过 REST API 验证记忆存储、查询、反馈、输入信息关联与分级迁移功能 | Wii |
...
| 编号 | 验收标准 |
| --- | --- |
| M6-AC-01 | SRD v1.2 全部 33 条需求（FR-DEP-001~004、FR-API-001~008、FR-UI-001~008、FR-DOC-001~002、PR-001~011）均经过验证，无"未验证"状态的需求条目 |
...
graph LR
    subgraph SRD["SRD v1.2 需求"]
        ...
        API["FR-API-001~008<br/>AI Agent 接口需求"]
