...
> 深度查询（`fast_only=false`）返回策略固定为: 关联记忆最多 5 条 + 热层匹配最多 3 条 + 冷层匹配最多 2 条, 总数最多 10 条。
...
| `query_memory` | `query: str`, `top_k: int = 10`, `fast_only: bool = False` | 查询语义相关记忆并返回 `input_id`；`fast_only=False` 时使用关联5+热3+冷2配额策略 |
...
```
GET /api/v1/memories?query=<查询词>&top_k=10&fast_only=false
```
...
```bash
curl "http://localhost:8080/api/v1/memories?query=用户界面偏好&top_k=10&fast_only=false"
```
...
sequenceDiagram
    ...
    Agent->>API: GET /api/v1/memories?query=...&fast_only=false
