...
def test_app_version_is_1_5_4() -> None:
    """APP_VERSION 应为 v1.5.4。"""
    from air_memory.main import APP_VERSION
    assert APP_VERSION == "1.5.4", f"APP_VERSION 应为 '1.5.4'，实际为 '{APP_VERSION}'"


def test_version_api_returns_1_5_4() -> None:
    """GET /api/v1/version 应返回版本号 1.5.4。"""
    response = client.get("/api/v1/version")
    assert response.status_code == 200
    assert response.json()["version"] == "1.5.4"
